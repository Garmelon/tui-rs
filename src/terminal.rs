    ...
    B: Backend,
{
    ...
    /// Flushes the backend, ensuring all changes are displayed. See
    /// [`Self::flush`] for more info.
    ...
    /// passes it to the current backend for drawing. To make sure the changes
    /// are displayed properly, [`Self::flush_backend`] must be called. To
    /// prepare for the next frame, [`Self::swap_buffers`] must be called.
